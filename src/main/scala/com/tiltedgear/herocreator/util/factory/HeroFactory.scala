...
import scalafx.scene.control.{Alert, ProgressBar}
...
object HeroFactory {
  ...
  val possibleRoles: List[String] = DB readOnly { implicit session =>
    SQL("select * from role").map(rs => rs.string("name")).list.apply()
  }

  val possibleFactions: List[String] = DB readOnly { implicit session =>
    SQL("select * from faction").map(rs => rs.string("name")).list.apply()
  }

  val possibleRaces: List[String] = DB readOnly { implicit session =>
    SQL("select * from race").map(rs => rs.string("name")).list.apply()
  }
